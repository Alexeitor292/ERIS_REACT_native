...
/** Incident Groups: read-only record view (shared grouping context across independent incidents). */
export default function EventGroupsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(e?.message ?? "Failed to load Incident Groups.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <AppShell title="Incident Groups">
      <div className="grid gap-4 p-4 md:p-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select value={status} onChange={(event) => setStatus(event.target.value as "ALL" | EventGroupStatus)} className="rounded-md border border-[var(--line)] bg-[var(--panel)] px-3 py-2 text-sm">
            <option value="OPEN">Open Incident Groups</option>
            <option value="CLOSED">Closed Incident Groups</option>
            <option value="ARCHIVED">Archived Incident Groups</option>
            <option value="ALL">All Incident Groups</option>
          </select>
        ...
        <div className="overflow-x-auto rounded-xl border border-[var(--line)] bg-[var(--panel)]">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={6} className="px-3 py-10 text-center text-sm text-muted">{busy ? "Loading Incident Groups…" : "No Incident Groups match the current filters."}</td></tr>
              ) : items.map((group) => (
                <tr key={group.id} className="border-b border-[var(--line)]/60 align-top last:border-b-0 hover:bg-[var(--panel-soft)]">
                  <td className="px-3 py-3"><Link to={`/incident-groups/${group.id}`} className="block text-[var(--ink)]"><div className="text-sm font-semibold hover:text-[var(--brand)]">{group.title}</div><div className="mt-0.5 text-xs text-muted">Incident Group #{group.id}</div></Link></td>
                  ...
                  <td className="px-3 py-3 text-right"><Link to={`/incident-groups/${group.id}`} className="rounded border border-[var(--line)] bg-[var(--panel)] px-2 py-1 text-[11px] font-semibold hover:bg-[var(--panel-soft)]">Open Incident Group</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
