    ...
    <AppShell title="User Administration">
      <div className="flex h-full flex-col gap-4 p-4 md:p-5">
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-[var(--line)] bg-[var(--panel)] p-4 md:flex-row md:items-center md:justify-between">
          <div><div className="text-sm font-semibold">ERIS access management</div><div className="mt-1 text-sm text-muted">Create accounts, manage operational roles, reset passwords, and disable access.</div></div>
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center">
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, email, role, or user ID" className={`${inputClass} md:max-w-xl`} />
          ...
          <button type="button" onClick={load} disabled={busy} className="rounded-md border border-[var(--line)] px-3 py-2 text-sm font-medium disabled:opacity-50">{busy ? "Refreshing…" : "Refresh"}</button>
        </div>
      ...
      {resetUser ? <PasswordResetDialog userName={resetUser.full_name} busy={busy} onClose={() => setResetUser(null)} onConfirm={resetPassword} /> : null}
    </AppShell>
  );
}
