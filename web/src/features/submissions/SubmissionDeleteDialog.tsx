...
export default function SubmissionDeleteDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4" role="presentation" onMouseDown={() => !busy && onClose()}>
      <section role="dialog" aria-modal="true" aria-labelledby="submission-delete-title" className="w-full max-w-lg rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-5 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className={`mt-4 rounded-xl border p-4 text-sm leading-6 ${submittedRecord ? "border-[color:color-mix(in_oklab,var(--bad)_45%,var(--line))] bg-[color:color-mix(in_oklab,var(--bad)_8%,transparent)]" : "border-[var(--line)] bg-[var(--panel-soft)]"}`}>
          {submittedRecord ? (
