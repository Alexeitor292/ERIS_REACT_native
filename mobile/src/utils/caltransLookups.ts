...
export const CALTRANS_DISTRICTS = Array.from(new Set(CALTRANS_COUNTIES.map((c) => c.district))).sort();

export function countyCodeFromNameOrCode(value?: string | null): string | null {
  const raw = (value || "").trim();
  if (!raw) return null;
  const upper = raw.toUpperCase();
  if (COUNTY_BY_CODE.has(upper)) return upper;
  const normalized = raw.replace(/\s+County$/i, "").trim().toLowerCase();
  return COUNTY_BY_NAME.get(normalized)?.code ?? null;
}

export function districtForCounty(value?: string | null): string | null {
  const code = countyCodeFromNameOrCode(value);
  if (!code) return null;
  return COUNTY_BY_CODE.get(code)?.district ?? null;
}

export function countyDisplayLabel(value?: string | null): string {
  const code = countyCodeFromNameOrCode(value);
  if (!code) return "";
