...
def content_signature(
    *,
    ...
    terrain_export_contract: str | None = None,
) -> str:
    ...
    ``imagery_export_contract`` does the same for the aerial-imagery export
    contract. ``terrain_export_contract`` identifies how the DEM is requested,
    georeferenced, independently verified, and packaged.

    Package identity must include backend content contracts, not only user-visible
    settings. Otherwise a device holding terrain generated by the former unverified
    export path would consider a corrected exact-extent package equivalent and never
    download it."""
    payload = {
    ...
    if terrain_export_contract is not None:
        payload["tx"] = str(terrain_export_contract)
