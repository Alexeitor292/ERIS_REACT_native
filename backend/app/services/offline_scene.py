...
def content_signature(
    *,
    ...
    imagery_export_contract: str | None = None,
) -> str:
    ...
    ``imagery_export_contract`` does the same for the aerial-imagery EXPORT CONTRACT.
    Package content identity must not be limited to user-visible settings: the tiled
    imagery registration defect was fixed purely in backend code, so with the same AOI,
    GISA data and road policy the old signature was byte-identical and a device holding
    the mis-registered package would never have re-downloaded the corrected one. Folding
    the contract identity in means any future change to what we request or how we verify
    it deterministically invalidates every stale imagery package."""
    payload = {
    ...
    if imagery_export_contract is not None:
        payload["ix"] = str(imagery_export_contract)
