...
class TestExpandRoles:
    ...
    def test_senior_specialist_expands_to_itself_only(self):
        # Routing v2's new role has NO legacy alias: inventing one would make
        # expand_roles() accept a name no database contains.
        assert roles.expand_roles(roles.GEOTECH_SENIOR_SPECIALIST) == ["GEOTECH_SENIOR_SPECIALIST"]
...
class TestMaintenanceVsOperational:
    ...
    def test_senior_specialist_is_operational_and_not_maintenance_only(self):
        user = {"id": 1, "roles": ["GEOTECH_SENIOR_SPECIALIST"]}
        assert roles.is_operational_user(user)
        assert not roles.is_maintenance_only(user)
        assert roles.has_canonical_role(user, roles.GEOTECH_SENIOR_SPECIALIST)
        # ...and it is its own role: a specialist is not an engineer, and an
        # engineer is not a specialist. The two eligibility rules depend on it.
        assert not roles.has_canonical_role(user, roles.GEOTECH_ENGINEER)
        assert not roles.has_canonical_role({"id": 2, "roles": ["FIELD_WORKER"]}, roles.GEOTECH_SENIOR_SPECIALIST)
