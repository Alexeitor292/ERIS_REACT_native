...
# List query has the same SELECT/FRAGMENT after the detail replacement, so patch
# the remaining occurrence separately.
text = replace_once(
    text,
    '              i.id, i.title, i.incident_type, i.description,\n',
    '              i.id, i.project_id, p.title AS project_title, p.status AS project_status, i.title, i.incident_type, i.description,\n',
...
# Legacy/direct engineer assignment should fail with an explicit Project error
# before the DB trigger is reached.
text = replace_once(
    text,
...
# Legacy coordinator-forward path must also enforce Project association.
text = replace_once(
    text,
